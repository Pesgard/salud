<script lang="ts">
	...
	import AgregarPaciente from '../../../components/pacientes/dataTable/modal/AgregarPaciente.svelte';
	...
	function modalComponentForm(): void {
		...
		const modal: ModalSettings = {
			...
			response: (r) => {
				console.log('response:', r); // r is the form data
				fetch('/dashboard/pacientes', {
					method: 'POST',
					headers: {
	...
	// Editar un paciente
	function modalComponentEdit(): void {
		const c: ModalComponent = { ref: AgregarPaciente };
		const modal: ModalSettings = {
			type: 'component',
			component: c,
...
<div class="max-w-full mx-auto space-y-4 flex flex-col items-center justify-center">
	...
	<div class="table-container p-4 w-full text-token space-y-4">
		...
		<table class="table table-hover table-compact table-auto w-full">
			<thead>
				<tr>
					<ThSort {handler} orderBy="idPaciente">No</ThSort>
				...
				<tr>
					<ThFilter {handler} filterBy="idPaciente" />
			...
			<tbody>
				{#each $rows as row}
					<tr>
						...
						<td>{row.email}</td>
						<td>
							<button class="btn variant-filled-surface hover:variant-soft-primary">

<script lang="ts">
    ...
    export let detallesBotiquin: any = [];
    console.log(detallesBotiquin);
    ...
    $: filteredKeys =
        Object.keys(data).length > 0
            ...
            : [];
    $: filteredKeys.push('medicamentos'); // Agrega 'medicamentos' a las claves filtradas para mostrarlos en la tabla
        ...
        if (printableElement) {
            ...
            if (printWindow) {
                ...
                printWindow.document.write(
                    '<style>body { font-family: Arial, sans-serif; } table { width: 100%; border-collapse: collapse; } th, td { border: 1px solid black; padding: 8px; text-align: left; } th { background-color: #f2f2f2; }</style>'
                );
            ...
            <thead>
                <tr class="border-b border-gray-600 bg-gray-700">
                    {#each filteredKeys as key}
                        <th
                            class="px-3 py-2 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider"
                        >
                            {key}
                        </th>
                    {/each}
                    ...
                    <tr class="border-b border-gray-600 dark:hover:bg-gray-600">
                        {#each filteredKeys as key}
                            <td class="px-3 py-1 text-sm text-gray-400">
                                {#if key === 'medicamentos'}
                                    <ul>
                                        {#each detallesBotiquin[index] as detalle}
                                            <li>
                                                {detalle.medicamento.nombre} ({detalle.medicamento.gramaje}{detalle.medicamento.tipo}) |  {detalle.cantidadUnitaria} Piezas
                                            </li>
                                        {/each}
                                    </ul>
                                {:else}

<script lang="ts">
	...
	// Propiedad para recibir los pacientes
	export let data: DeficienciVisualFormulario;
	console.log(data);

	const pacienteNombre: string = data.pacienteNombre.firstName + ' ' + data.pacienteNombre.lastName; //variable para el nombre del paciente

	//variables de los checks

	let examenVistaCheck = data.vista !== '' || data.vistaDate !== null;
	let examenAuditivoCheck = data.auditivo !== '' || data.auditivoDate !== null;
